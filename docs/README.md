...
```bash
composer require stonehilt/openapi
```
...
'providers' => [
    // ...
    StoneHilt\OpenApi\OpenApiServiceProvider::class,
];
```
...
```bash
php artisan vendor:publish --provider="StoneHilt\OpenApi\OpenApiServiceProvider" --tag="openapi-config"
```
